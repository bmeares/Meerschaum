...
- **Scope the `dbc_dark` theme so it no longer leaks onto every Dash page.**  
  ...
  To keep the web console (including its body-portaled dropdown menus) unchanged, the Dash app now sets `class="dbc_dark"` on `<body>` by default. A plugin page can opt out with `@web_page(dark_theme=False)` — the Web Console then removes the `dbc_dark` class from `<body>` while that page is active (restoring it elsewhere, including on in-app navigation), so the plugin's own styling applies without competing with the theme. See [Styling Your Page](/reference/plugins/writing-plugins/#styling-your-page).

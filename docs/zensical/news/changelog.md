...
- **Add a first-class MCP server.**  
  Meerschaum now ships a [Model Context Protocol](/reference/mcp/) server, so an AI assistant can discover your pipes, read their data, and run actions. It replaces the community `mcp` plugin — uninstall it with `mrsm uninstall plugin mcp`.

    - **Run it locally over stdio.**  
      `mrsm start mcp` speaks MCP on stdin/stdout, for a local install with no API server and no token:
    ...
    - **Or remotely over HTTP.**  
      An API instance serves the same tools at `/mcp`, where each one requires the same [scope](/reference/api-instance/tokens/) as its equivalent REST route. Tools your token cannot call are hidden entirely, so an assistant with a `pipes:read` token only ever sees the read tools:

      ```bash
      mrsm register token --label agent --scopes pipes:read
      ```

    - **15 tools, 5 resources, and 3 prompts.**  
      Read and write pipes (`list_pipes`, `get_pipe_data`, `get_pipe_stats`, `sync_pipe`, `register_pipe`, …), query a connector with `read_sql`, or run any action with `execute_action`. Resources expose the pipe registry and a local copy of this reference at `mrsm://docs`; prompts add guided `bootstrap_pipe`, `diagnose_sync_failure`, and `explain_pipe` workflows. See the [tool reference](/reference/mcp/tools/).

    - **Lock it down for shared deployments.**  
      Set `read_only` to expose only tools which never modify data, whatever a token allows. `execute_action` refuses `sh`, `os`, and `python` by default, and `read_sql` refuses anything that is not a single read-only statement:
      ...
      See [MCP security](/reference/mcp/security/) before exposing `/mcp` to untrusted callers.

- **Reject `json` columns used as index columns.**  
  JSON values are dictionaries or lists, which cannot be sorted or deduplicated when syncing — previously the first sync succeeded and the second failed with `TypeError: unhashable type: 'dict'`. Registering, editing, or syncing such a pipe now fails immediately with the reason:

  ```python
  import meerschaum as mrsm

  pipe = mrsm.Pipe(
      'demo', 'satellites', instance='sql:main',
      columns={'datetime': 'ts', 'id': 'display_config'},
  ...
  Declare the column under [`indices`](/reference/pipes/parameters/#indices) instead if you only need a database index on it — on PostgreSQL-like flavors it is stored as `JSONB`, which supports B-tree, hash, and GIN indices. See [Data Types](/reference/pipes/dtypes/#json-columns-cannot-be-index-columns).

- **Require authentication for web console actions which read or write data.**  
  Twelve Dash callbacks — querying and downloading pipe data, editing parameters and SQL definitions, syncing documents, the users listing, job log downloads, and token management — could be triggered without a session by posting directly to the web console's callback endpoint. They now check the session first, and token operations enforce the same ownership rule as the REST routes.
...
- **Add Plotly Dash's MCP server for plugin authors.**  
  Dash can expose the web console's layout at `/dash/_mcp` so an assistant can inspect the component tree while you write a [web page plugin](/reference/plugins/types-of-plugins/). It is a development aid, separate from Meerschaum's own MCP server, and disabled by default because it is served without authentication:
  ...
  Only layout introspection is exposed; a callback must opt in with `@dash_app.callback(..., mcp_enabled=True)`. See [Web Console](/reference/api-instance/web-console/#dash-mcp-server).

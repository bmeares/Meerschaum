...
- **Add a first-class [MCP server](/reference/mcp/).**  
  ...
  This replaces the community `mcp` plugin — uninstall it with `mrsm uninstall plugin mcp`. Read [MCP security](/reference/mcp/security/) before exposing `/mcp`: `api:mcp:read_only` hides every modifying tool, `execute_action` denies `sh`, `os`, and `python` by default, and `read_sql` accepts only a single read-only statement.
